...
LIBNAME NY "optimal_tradeoffs_code\data\New York Data\SAS\";
LIBNAME NYRAW "optimal_tradeoffs_code\data\New York Data\SAS\RawData\";


* Use for debugging;
options nosymbolgen errors=0;

%MACRO ProcNY(rawname=,yr=);
%put Input file: &rawname;

%let fname = ny&yr;

* Classify by procedure;
%LET INNAME = NY.&fname.IN;
...
data &OUTNAME;
	...
	* classify procedure codes;
	...
	array proc_cd_{15} $ PRINPROC OTHPROC1-OTHPROC14;

	/* 1 to 15 for all codes, 1 to 1 for just principal procedure */
	do I=1 to 15;
		...
		if proc_cd_(i) ge '537' and proc_cd_(i) le '5384' then hrn_othabd = 1;
...
%INCLUDE 'optimal_tradeoffs_code\data_assembly\comoanaly_hcup_ny.sas';


* Include comorbidity measures in data set;
%LET INNAME = &fname.CODED;
%LET OUTNAME = NY.&fname.GS;
data &OUTNAME;
    MERGE &INNAME
        &fname.comorb;
    by dischNum;
...
PROC EXPORT DATA = NY.&OUTNAME
    OUTFILE= "optimal_tradeoffs_code\data\New York Data\STATA\&OUTNAME..dta"
    DBMS=STATA REPLACE;
RUN;
%put ...saved to STATA;
...
%ProcNY(rawname=LIMITEDIP07,yr=2007);
%ProcNY(rawname=LIMITEDIP08,yr=2008); 
%ProcNY(rawname=LIMITEDIP09,yr=2009); 
%ProcNY(rawname=LIMITEDIP10,yr=2010); 
%ProcNY(rawname=LIMITEDIP11,yr=2011);  
